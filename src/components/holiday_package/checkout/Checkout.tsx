import React, { useEffect, useState } from "react";
import Navbar from "./components/Navbar";
import TravelDetails from "./components/TravelDetails";
import PackageDetails from "./components/PackageDetails";
import Cancellation from "./components/Cancellation";
import Bill from "./components/Bill";
import { useMediaQuery } from "react-responsive";
import CheckoutHeroSection from "./components/CheckoutHeroSection";
import About from "../../home/about/About";
import Footer from "../../home/footer/Footer";

const checkoutDetails = {
  tripName: "Delightful Shimla & Manali Vacay",
  tripFrom: "01 March 2024",
  tripTo: "07 March 2024",
  travelDetails: {
    travellers: [
      {
        name: "Shubh Mangukiya",
        profileComplete: true,
        profilePic:
          "data:image/png;base64,",
      },
      { name: "Shubham Kumar Mahato", profileComplete: false },
    ],
    number: 9099331142,
    countryCode: "+91",
    email: "shubh.mangukiya01@gmail.com",
    pincode: 390019,
    specialRequests: "",
    address: "",
  },
  packageDetails: {},
  cancellationAndDateChange: {},
  bill: {
    people: 2,
    discountPercentage: 15,
    cost: 22316,
    originalCost: 24360,
    discount: 3200,
    tax: 1156,
  },
};

const Checkout = () => {
  const [checkoutData, setCheckoutData] = useState<any>();
  const isAboveMediumScreen = useMediaQuery({ minWidth: 768 });

  useEffect(() => {
    const setCheckout = () => {
      if (!checkoutDetails) return;
      setCheckoutData(checkoutDetails);
    };
    setCheckout();
  }, []);

  return (
    <div>
      {isAboveMediumScreen ? (
        <Navbar
          data={{
            name: checkoutData?.tripName,
            time: {
              from: checkoutData?.tripFrom,
              to: checkoutData?.tripTo,
            },
          }}
        />
      ) : (
        <CheckoutHeroSection
          data={{
            name: checkoutData?.tripName,
            time: {
              from: checkoutData?.tripFrom,
              to: checkoutData?.tripTo,
            },
            bill: checkoutData?.bill,
          }}
        />
      )}
      <div className="xl:pl-[37px] pb-[40px] xl:pb-[60px] px-[25px] xl:pr-[55px] flex gap-[30px]">
        <div className="mt-[25px] flex-1 space-y-[18px] xl:space-y-[25px]">
          <TravelDetails
            data={checkoutData?.travelDetails}
            setCheckoutData={setCheckoutData}
          />
          <PackageDetails />
          <Cancellation />
        </div>
        {isAboveMediumScreen && <Bill data={checkoutData?.bill} />}
      </div>
      <About />
      <Footer />
    </div>
  );
};

export default Checkout;
